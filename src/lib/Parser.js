...
class Parser {
				...
				const li = token.items.map((item) => {
					const children = item.tokens.flatMap((cItem) => {
						if (cItem.type === "text") {
							...
							const childTokens = cItem.tokens || [];
							const listChildren =
	...
	getNormalizedSiblingNodesForBlockAndInlineTokens(tokens, textStyle) {
		let tokenRenderQueue = [];
		const siblingNodes = [];
	...
	hasDuplicateTextChildToken(token) {
		if (!("tokens" in token)) {
			return false;
		}
		if (
